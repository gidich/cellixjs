...
const storageAccountName = process.env["STORAGE_ACCOUNT_NAME"]; // need to throw an error if this is not set
if (!storageAccountName) {
  throw new Error("Missing required environment variable: STORAGE_ACCOUNT_NAME");
}

const storageAccountKey = process.env["STORAGE_ACCOUNT_KEY"]; // need to throw an error if this is not set
